...
test("provides first-class presets for header color and version checks", () => {
  const setHeaderColorItem = API_ITEMS.find((item) => item.id === "setHeaderColor");

  expect(setHeaderColorItem?.presets).toEqual(["#18A0FB", "#101010", "#FFFFFF"]);
  expect(VERSION_PRESETS).toEqual(["1.0.0", "2.0.0", "3.2.0"]);
  expect(setHeaderColorItem?.defaultParams).toEqual({ color: "#18A0FB" });
});
